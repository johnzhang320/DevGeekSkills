...
/*
 *  Different from Abstract SessionFactory , Spring Configured SessionFactory must add all scan modeling package 
 *  in one Spring annotation configuration file or XML configuration file, one we run hin=bernate.hbm2ddl.auto = create 
 *  drop all tables and create again unless set configuration file per each model package because Inject parameter must be 
 *  hard code in configuration file or use JPA not to need to specify model package
 *  
 *  Make sure 
 *  (1) Run below code soon after xtoyDataInititialTest 
 *  (2) run it in JRE 1.8
 */
