...
/**
 * LocaleMsg retrieve message from locale property files by message code, 
 * which will be used in front end validation and  back end validation
 *  
 * We define one types of property files for International language environment:
 * application_xx.propertie --message with arguments (Sometimes arguments must be
 * international too) and form field labels
 * We define:
 * required.field ={0} is required
 * invalid.value.field ={0} is invalid {1}
 * invalid.field ={0} is invalid 
 * Purposes: 
 * (1) We will simplify the message definition and eliminate the a 
 *     lot of repeating information
 * (2) Pass locale message fetched from locale message code as arguments
 *     to form international message sentence.
 * 
 */
...
public class LocaleMsg {
	private static Logger LOG = Logger.getLogger(LocaleMsg.class);
	 ...
	 protected static void loadProperties(String language) {		
         ...
         applicationProperties = lp.getProperties("application_"
                                         +LocaleMsg.lang +".properties");
     }
	/**
	 ...
	 */
     ...
     public static String getMessage(String messageCode,String msgArg1,
      	...
      	return messageSentence.replace("{0}", msgArg1).replace("{1}",
      			                      msgArg2).replace("{2}", msgArg3);
     	 
  	 } 
     
     /**
      * Form message sentence from message code , List<String> Arguments Message 
      * Code for international arguments 
      * @param messageCode
      * @return
      */
     ...
     /**
      * Form message sentence from message code , List<String> Arguments Code for 
      * international arguments and Request
      * @param messageCode
      * @return
      */
     
     public static String getMessageFromCode(HttpServletRequest request, String messageCode,
    		                                 List<String> msgCodeArgs ) {
    	if (null ==LocaleMsg.request) {
    		initProperties(request);
    	}
    	String messageSentence =  applicationProperties.getProperty(messageCode);
     ...
     /**
      * Form message sentence from message code , String Arguments Code1 for international
     ...
     /**
      * Form message sentence from message code , String Arguments Code1,Code2 for 
      * international arguments and Request
      * @param messageCode
      * @return
      */
     ...
     /**
      * Form message sentence from message code , String Arguments Code1,Code2,Code3 for 
      * international arguments and Request
      * @param messageCode
      * @return
      */
     ...
     public static String getMessageFromCode( String messageCode,String msgCodeArg1, 
    		                                  String msgCodeArg2,String msgCodeArg3) {
    	if (null ==LocaleMsg.request) {
    		initProperties(request);
    	}
    	String messageSentence =  applicationProperties.getProperty(messageCode);
