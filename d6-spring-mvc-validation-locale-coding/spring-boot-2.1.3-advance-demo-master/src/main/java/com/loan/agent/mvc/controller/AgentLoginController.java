...
public class AgentLoginController  {
	...
	public String processSubmit(
		...
		if (result.hasErrors()) {
			...
			encrypted =agentTable.getCreditNumber();
			
			decrypted = KeyPairManager.getInstance().decrypt(encrypted);	
			agentTable.setDecryptedCredit(decrypted);
			agentTable.setEncryptedCredit(encrypted);
			
			encrypted =agentTable.getSecurityCode();
			decrypted = KeyPairManager.getInstance().decrypt(encrypted);	
			agentTable.setDecryptedSecurityCode(decrypted);
			agentTable.setEncryptedSecurityCode(encrypted);
			
			encrypted =agentTable.getSocialSecurity();
			decrypted = KeyPairManager.getInstance().decrypt(encrypted);	
			agentTable.setDecryptedSSO(decrypted);
			agentTable.setEncryptedSSO(encrypted);
