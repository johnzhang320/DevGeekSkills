...
public class Oath2LoginController  {
    Logger LOG = Logger.getLogger( Oath2LoginController.class);
    @Autowired
	...
	@RequestMapping(value="/oauth2Login.html",method = RequestMethod.POST)
	public String processSubmit(
		...
		if (result.hasErrors()) {
		     ...
		     */
		    return "redirect:oauth2ShowToken.html"; 
		}
		 
	}
	
	@RequestMapping( value = "/oauth2ShowToken.html", method = RequestMethod.GET)	
	public String agentLoginSuccess (Model model, HttpServletRequest request) throws Exception {
		LOG.info("Oauth2ShowToken () begin");
		...
		model.addAttribute("agentTable",agentTable);
		LOG.info("Oauth2ShowToken () end");
		/**
		 *  Mapping Handler: AgentSignupSuccessis defined in tiles-spring.xml
		 */
		return "Oauth2ShowToken";
	}
	
	@RequestMapping(value="/oauth2Client.html",method = RequestMethod.POST)
