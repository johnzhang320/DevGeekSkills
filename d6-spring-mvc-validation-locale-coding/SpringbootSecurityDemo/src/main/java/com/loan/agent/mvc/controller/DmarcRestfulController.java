...
public class DmarcRestfulController {
    ...
    @RequestMapping(value = "/mongodb/masters.html/", method = RequestMethod.GET,produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<List<DBObject>> findRangeMasterReportsMongo() {
        ...
        if(results.isEmpty()){
            ...
            return new ResponseEntity<List<DBObject>>(HttpStatus.NO_CONTENT);//You many decide to return HttpStatus.NOT_FOUND
        }
      
        return new ResponseEntity<List<DBObject>>(results, HttpStatus.OK);
    }
