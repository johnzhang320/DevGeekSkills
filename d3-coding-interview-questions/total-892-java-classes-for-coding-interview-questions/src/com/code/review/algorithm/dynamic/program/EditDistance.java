...
public class EditDistance {
	...
	// Iterative way
		public int editDistanceIterative(String word1, String word2) {
			...
			// row[0] initilize from column value as string index
			for (int j=0;j<=n; j++) {
				dp[0][j]=j;
			}
			// col[0] initialize from row value as another string index
			for (int i=0;i<=m;i++) {
				dp[i][0] = i;
			}
			for (int i=0;i<m; i++) {
				for (int j=0;j<n; j++) {
					...
					if (word1.charAt(i) == word2.charAt(j)) {
						// if chars from two words are same , we take diagonal dp[i+1][j+1]=dp[i][j]
						dp[i+1][j+1] = dp[i][j];
					} else {
						// if chars from two words are not same , min between last dp[i][j] and min of neigthor(up and left)
						dp[i+1][j+1] = Math.min(
