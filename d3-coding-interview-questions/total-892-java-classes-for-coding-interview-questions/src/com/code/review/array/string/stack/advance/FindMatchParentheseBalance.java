...
public class FindMatchParentheseBalance {
	public static boolean balanceParent(String s) {
		...
		for (int i=0; i<s.length();i++) {
			char c = s.charAt(i);
			if (map.containsKey(c)) {   // ( or [ 
				stack.push(map.get(c));  // push ( or [ close parenthese to stack
				open=true;
			} else {
				if (!stack.isEmpty() && c==stack.peek()) {  // if really find the close one, means balance, pop
					stack.pop();
				}
			}
		}
		if (!open) {
			return true;  // ensure empty string is balance
		}
		return stack.isEmpty();  // stack not empty means not balance
	}
