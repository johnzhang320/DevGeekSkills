	...
	public void put(final Key key, final Value value) {
	    if (map.containsKey(key)) {
	        queue.remove(key); // remove the key from the FIFO queue 
	                           // need know how to delete key from queue, double linked list is O(1) for deletion
	    }

	    while (queue.size() >= maxSize) {
	        Key oldestKey = queue.poll();   // pull element from head of queue
	        if (null != oldestKey) {
	            map.remove(oldestKey);   
	        }
	    }
	    queue.add(key);   // insert element node at tail of linked queue
	    map.put(key, value);
	}
