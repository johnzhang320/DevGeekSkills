...
public class LongestCommonSequenceTwoString {
	 ...
	 *  dp[i][0]=0
	 *  if (str1[i] == str2[j]
	 *     dp[i][j] = 1 + dp[i-1][j-1]  pick up diagonal value if match, means LCS increased
	 *  else 
	 *     dp[i][j] = max(dp[i-1][j] , dp[i][j-1]) if not match pick up max of left or up value, means max LCS is not changefr
	 *  
