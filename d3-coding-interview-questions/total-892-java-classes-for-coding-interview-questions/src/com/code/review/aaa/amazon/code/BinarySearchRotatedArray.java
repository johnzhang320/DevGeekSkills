...
public class BinarySearchRotatedArray {
	 ...
	 *  amazon-interview-questions
		...
		My Approach:
		(1) while (low<=high), check if target == A[mid] return mid
		(2) if A[low] < A[mid] means in A[low] to A[mid] is ascend , therefore , 
		(3) if target is within range A[low] to A[mid] , high = mid-1 , if out of A[low] to A[high]
		low=mid+1
		(4) if target is  within range A[mid] to A[high] , low = mid+1 , if with A[low] to A[high], high=mid-1
