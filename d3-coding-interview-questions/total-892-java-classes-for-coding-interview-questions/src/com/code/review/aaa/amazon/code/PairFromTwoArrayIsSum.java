...
public class PairFromTwoArrayIsSum {
	...
	public static void findPaireIsSumFrom2Arrays(int A[],int B[], int sum) {
		...
		for (int i=0; i<A.length;i++) {
			result.put(sum-A[i],A[i]);
		}
		for (int i=0;i<B.length;i++) {
			if (result.containsKey(B[i])) {
				System.out.println("("+B[i]+","+result.get(B[i])+")");
			}
		}
	}
	...
	public static void main(String[] args) {
		...
		int A[] = {1,2,4,-1,5,7,9};
		int B[] = {3, 6, 3, 4, 0}; 
		findPaireIsSumFrom2Arrays(A,B, 5);
	}
