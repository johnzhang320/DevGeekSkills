...
public class SendingMessageTimeSum {
  ...
  Emp9(3)   Emp10(1)   Emp11(2)  EMP12(4)
	...
	public void findMaxTopBottomTime(Node root,int sum) {
		if (root.belows==null) {
			
			return;
		}
		
		for (Node n: root.belows) {
			
			
			findMaxTopBottomTime(n,sum+n.sendingTime);
			
		}
		//System.out.println("\n second bottom:"+root.empName+"("+sum+")");
		max = Math.max(sum, max);   // last node which has children path summary from top to it
	}
	...
	public void findMaxTopBottomTimePath(Node root,int sum,List<String> list) {
		if (root.belows==null ) {
 			return;
		}
		...
		for (Node child: root.belows) {
			...
			findMaxTopBottomTimePath(child,sum+child.sendingTime,list);
			list.remove(list.size()-1);  // reach node.belows =null , back trace to parent
	 	}
		if (sum>max) {     // sum order is same of calling stack storing order
	...
	public static void main(String[] args) {
		...
		f.findMaxTopBottomTimePath(root, root.sendingTime, list);
		System.out.println("Max Sending time="+f.max+"\nPath="+f.result);
