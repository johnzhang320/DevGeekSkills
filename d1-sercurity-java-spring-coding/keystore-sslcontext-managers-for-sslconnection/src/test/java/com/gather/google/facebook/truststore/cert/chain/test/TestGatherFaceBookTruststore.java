...
public class TestGatherFaceBookTruststore {
	static Logger Log = Logger.getLogger(TestGatherFaceBookTruststore.class);
	...
	public void test1_collectCMServerCertChainTest() {
		...
		String pem = Utils.readFileExactly(WORkING_DIR+"facebookTrustCertChain.pem");
		Log.info("facebookTrustCertChain.pem shown below");
		System.out.println(pem);
		
		Log.info("TestCase 2 showFacebookTrustCertInJKSFormat() ");
		String keystorefile=WORkING_DIR+("facebookTrustCertChain.jks");
		String command = " -list "+
                 " -v "+
