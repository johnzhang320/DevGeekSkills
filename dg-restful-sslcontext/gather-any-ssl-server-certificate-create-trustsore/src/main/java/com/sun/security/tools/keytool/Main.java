...
/**
 *  Modified By John Zhang , john.zhang320@gmail.com , 03/02/2018
 *  Source Code URL: http://hg.openjdk.java.net/jdk8/jdk8/jdk/file/b5aad88cbf12/src/share/classes/sun/security/tools/keytool/Main.java
 *  (1) Add the Exception catch for  keyStore.load(ksStream, storePass); if password is wrong or keystore was tempered
 *  (2) debug=true
 *  (3) Make jdk access restrict in eclipse
 *      right click project --> Properties-->Java Build Path --> Libraries--> JRE System Library --> Access Rule --> Restrict --> **
 *  (4) in doPrintEntry() function does not find sun.security.tools.keytool.Resources, 
 *   private static final java.util.ResourceBundle rb =
        java.util.ResourceBundle.getBundle(
            "com.sun.security.tools.keytool.Resources");
        we must change it to com.sun.security.tools.keytool.Resources
        download Resources.java from https://github.com/JetBrains/jdk8u_jdk/blob/master/src/share/classes/sun/security/tools/keytool/Resources.java
 */
...
import static com.sun.security.tools.keytool.Main.Command.*;
import static com.sun.security.tools.keytool.Main.Option.*;
...
public final class Main {
    ...
    void doCommands(PrintStream out) throws Exception {
         ...
         */
       /* if (!nullStream) {
        		Log.debug("Loading keystore stream with passed password");
		...
		*/
        Log.debug("Loading keystore stream with passed password");
        try {  // catch exception if password is wrong John Zhang 01/03/2018
	        if (!nullStream) {
	            keyStore.load(ksStream, storePass);
	            if (ksStream != null) {
	            		Log.debug("Loading keystore stream successfully");
	                ksStream.close();
	            }
	        }
        } catch (Exception e) {
       		// all display message in screen will be received into string in above caller in Caller DoKeyTool John Zhang 01/03/2018
        		Log.debug("Loading keystore stream failed!");
        ...
        throws Exception
    {
        ...
        if (storePass == null
            ...
            printWarning();
        		...
        		Log.debug("not windows storeType");
            out.println();
        }
