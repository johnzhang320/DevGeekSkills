...
1. I have created a Ruby on Rails plugin which uses your jquery plugin for the front-end. There is a problem with a form authentication_token that rails uses to see if a form has been tampered with. The problem stems from your plugin encrypting all form input elements. The problem goes away if i don't encrypt this particular hidden <input> element. I have made a minor modification to your plugin to accept a new option which will ignore a specified selector. I am wondering if you would be willing to look at the change and consider incorporating it into your plugin?
...
(function($) {
        $.jCryption = function(el, options) {
                ...
                base.init = function() {
                  ...
                  /* change is here, the other change is in options */
                                                                         encryptSelector = 'input';
                                                                         if( base.options.dontEncryptSelector.length)
                                                                           encryptSelector += ':not('+base.options.dontEncryptSelector+')';
                                                                               
                 $(base.$el).find(encryptSelector).attr("disabled",true).end().append($encryptedElement).submit();
                 /* to here */
...
//                                                       $(base.$el).find(":input").attr("disabled",true).end().append($encryptedElement).submit();
        ...
        /** 
         *   Based on http://api.jquery.com/jquery.getjson/
		 ...
		 *		success: success
		 *	});
		 *  which could return without server success data be sent back, therefore this code can not be called in submit function.
         *  So we Chang $.getJSON to $.ajax which contain async :false and timeout, ensure this function doesn't return until really get public key 
         *  Then we are allow to call it in javascript form submit function.
         */
        $.jCryption.getPublicKey = function(url,callback) {
            ...
            base.getKeys = function() {
               $.ajax({
    	 			...
    	 			dataType: "json",	 
    	 			url: url,		
    	 			async: false,    // hold on return util server response
    	 			timeout:800000,
            ...
            // Based RSA theory:  {e,n} is public key , {d,n} is private key, md=(p-1) x (q-1) , n=p x q, p and q must be prime number
            // Therefore return is public instead of keypair
            base.jCryptionKeyPair = function(encryptionExponent, modulus, maxdigits) {
                    setMaxDigits(parseInt(maxdigits,10));
                    this.e = biFromHex(encryptionExponent);
                    this.m = biFromHex(modulus);
        ...
        $.jCryption.defaultOptions = {
                ...
                dontEncryptSelector:""    /* other change is here */
        };
