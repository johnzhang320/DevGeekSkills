 <%
   String jQueryPath =request.getContextPath()+"/BuildJQuery/development-bundle/";
    String imagePath =request.getContextPath()+"/images/layoutImages/";
 %>
 
 <link rel="stylesheet" href="<%=jQueryPath%>themes/redmond/jquery-ui-1.8.23.custom.css"/> 
  <script type="text/javascript" src="<%=jQueryPath%>jquery-1.8.0.js"></script>
  <script type="text/javascript" src="<%=jQueryPath%>jquery.jcryption-1.1.js"></script>
  <script type="text/javascript" src="<%=jQueryPath%>/ui/jquery-ui-1.8.23.custom.js"></script>
  <script type="text/javascript" src="<%=jQueryPath%>/security/jquery.sha256.js"></script>
    
   <script type="text/javascript" src="<%=request.getContextPath()%>/dwr/engine.js"> </script>  
  <script type="text/javascript" src="<%=request.getContextPath()%>/dwr/util.js"> </script>
  <script type="text/javascript" src="<%=request.getContextPath()%>/dwr/interface/StateCountyManager.js"> </script>
 
 
        
   <link rel="stylesheet" type="text/css" href="<%=request.getContextPath()%>/css/common/layout.css" />
   <link rel="stylesheet" type="text/css" href="<%=request.getContextPath()%>/css/common/utilities.css" />
   <link rel="stylesheet" type="text/css" href="<%=request.getContextPath()%>/css/menufiles/dropDownMenu.css" /> 
 
   
   <script type="text/javascript" src="<%=request.getContextPath()%>/js/form/formUtils.js"> </script>
   
   <script type="text/javascript" src="<%=request.getContextPath()%>/js/utils/dialog.js"> </script>
   
 <!--   <script type="text/javascript" src="<%=request.getContextPath()%>/js/utils/goBackAgentLogin.js"> </script>-->
   
   